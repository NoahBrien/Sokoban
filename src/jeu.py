...
class Jeu:
    def __init__(self):
        """
        Initialise le jeu principal, charge les paramètres, les ressources
        et configure l'état initial du jeu.
        """
        ...
        self.deadlock_detected_this_attempt = False

        self._recalculer_taille_case_effective()

    def _recalculer_taille_case_effective(self):
        """
        Recalcule la taille effective des cases en fonction du facteur de zoom
        et régénère les images zoomées dans l'interface graphique.
        """
        self.taille_case_effective = int(C.TAILLE_CASE * self.zoom_factor)
    ...
    def appliquer_zoom(self, zoom_direction, pos_souris_ecran_absolue=None):
        """
        Ajuste le facteur de zoom du jeu et met à jour la taille effective des cases.
        Prend en compte la position de la souris pour un zoom centré (non implémenté ici).
        """
    ...
    def appliquer_volume_sons(self):
        """
        Applique le volume sonore actuel à tous les effets sonores chargés.
        """
        if self.sons:
    ...
    def set_volume(self, nouveau_volume):
        """
        Définit un nouveau volume pour les sons du jeu et sauvegarde ce paramètre.
        """
    ...
    def toggle_textures(self):
        """
        Active ou désactive l'affichage des textures et sauvegarde ce paramètre.
        """
    ...
    def rafraichir_listes_niveaux(self):
        """
        Recharge les listes des chemins vers les niveaux par défaut et personnalisés.
        Réinitialise également les boutons de sélection de niveau dans l'interface.
        """
    ...
    def _adapter_taille_fenetre_pour_niveau(self):
        """
        Ajuste la taille de la fenêtre du jeu en fonction de l'état actuel
        (menu, jeu, éditeur) et des dimensions du contenu à afficher.
        """
    ...
    def charger_niveau_par_specification(self, spec_niv):
        """
        Charge un niveau en utilisant une spécification (dictionnaire) contenant
        le chemin du fichier, l'index global et le nom à afficher.
        """
    ...
    def charger_niveau_temporaire_pour_test(self, contenu_str, editeur_ref):
        """
        Charge un niveau à partir d'une chaîne de caractères (généralement depuis l'éditeur)
        pour un test rapide, en conservant une référence à l'éditeur.
        """
    ...
    def reinitialiser_etat_niveau(self):
        """
        Réinitialise l'état du niveau actuel (mouvements, temps, victoire, etc.)
        et l'état des indicateurs d'indice et d'impasse.
        """
    ...
    def executer(self):
        """
        La boucle principale du jeu. Gère le temps, les événements,
        la mise à jour de la logique et le dessin, jusqu'à ce que le jeu se termine.
        """
    ...
    def demander_indice(self):
        """
        Calcule et affiche un indice pour le prochain mouvement en utilisant un solveur BFS.
        Met en cache la solution pour l'état actuel pour éviter des recalculs inutiles.
        """
        ...
        if self.hint_solution_pour_etat_actuel and len(self.hint_solution_pour_etat_actuel) > 0:
            ...
            dx, dy, dir_txt = 0, 0, "?"
            if mvt_sym == 'H': dx, dy, dir_txt = 0, -1, "Haut"
            elif mvt_sym == 'B': dx, dy, dir_txt = 0, 1, "Bas"
            elif mvt_sym == 'G': dx, dy, dir_txt = -1, 0, "Gauche"
            elif mvt_sym == 'D': dx, dy, dir_txt = 1, 0, "Droite"
            ...
            self.hint_message = "Aucun indice (BFS)."
            self.hint_next_move_display_pos = None
            self.hint_solution_pour_etat_actuel = None

    def gerer_evenements(self):
        """
        Traite tous les événements Pygame (clavier, souris, fermeture de fenêtre)
        en fonction de l'état actuel du jeu (menu, jeu, éditeur, etc.).
        """
        for event in pygame.event.get():
            ...
            if self.etat_jeu == C.ETAT_JEU or self.etat_jeu == C.ETAT_EDITEUR:
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if event.button == 4: self.appliquer_zoom(1, event.pos)
                    elif event.button == 5: self.appliquer_zoom(-1, event.pos)
            ...
            if self.etat_jeu == C.ETAT_MENU_PRINCIPAL:
                if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                    ...
                    if n_e:
                        if n_e == C.ETAT_QUITTER: self.fonctionnement = False
                            ...
                            self.etat_jeu = n_e
                            self.zoom_factor = 1.0; self._recalculer_taille_case_effective()
                            self._adapter_taille_fenetre_pour_niveau()
                ...
                if not self.animation_active:
                    if event.type == pygame.KEYDOWN:
                        if event.key != pygame.K_h:
                            self.hint_next_move_display_pos = None; self.hint_message = ""; self.hint_solution_pour_etat_actuel = None
                        ...
                        if not self.victoire_detectee:
                            ...
                            if event.key in key_map:
                                ...
                                if self.niveau_actuel.deplacer_joueur(*dx_dy, self.taille_case_effective, self.sons,
                                    ...
                                    if not self.deadlock_detected_this_attempt and self.niveau_actuel and self.niveau_actuel.joueur:
                                        etat_apres_mvt = self.niveau_actuel.get_etat_pour_solveur()
                                        if etat_apres_mvt:
                                            niveau_sim_pour_bfs = classes.Niveau(nom_fichier_ou_contenu=self.niveau_actuel.contenu_initial_str,
                                                                                est_contenu_direct=True)
                                            niveau_sim_pour_bfs.appliquer_etat_solveur(etat_apres_mvt)
                                            solveur_bfs_check = solveurs.SolveurBFS(etat_apres_mvt)
                                            solution, _iterations = solveur_bfs_check.resoudre(niveau_sim_pour_bfs)
                                            if solution is None:
                                                self.deadlock_message = "Il n'y a sûrement pas de moyen de finir le niveau."
                                                self.deadlock_detected_this_attempt = True
                                            else:
                                                self.deadlock_message = ""
                                        else:
                                             self.deadlock_message = ""
                                ...
                                if self.niveau_actuel.annuler_dernier_mouvement():
                                    ...
                                    if not self.deadlock_detected_this_attempt:
                                        self.deadlock_message = ""

                        if event.key == pygame.K_h: self.demander_indice()
                        elif event.key == pygame.K_r: self.reinitialiser_etat_niveau()
                            ...
                            if self.mode_test_editeur:
                                ...
                                self.etat_jeu = C.ETAT_MENU_PRINCIPAL
                            self.hint_next_move_display_pos = None; self.hint_message = ""; self.deadlock_message = ""; self.hint_solution_pour_etat_actuel = None
                            self._adapter_taille_fenetre_pour_niveau()
                ...
                if event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE:
                    if not (self.interface_graphique.editeur_instance and self.interface_graphique.editeur_instance.input_nom_actif):
                        self.etat_jeu = C.ETAT_MENU_PRINCIPAL
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_SPACE: self.avancer_pas_solveur()
                        ...
                        self.solveur_solution_pas = None; self.niveau_pour_visualisation = None; self.etat_avant_solveur = None; self.solveur_iterations = 0

    def mettre_a_jour(self):
        """
        Met à jour la logique du jeu, comme la vérification des conditions de victoire
        ou la gestion de l'état de la visualisation du solveur.
        """
        if self.etat_jeu == C.ETAT_JEU and self.niveau_actuel:
            if not self.animation_active and not self.victoire_detectee:
                if self.niveau_actuel.verifier_victoire():
                    ...
                    self.hint_next_move_display_pos = None; self.hint_message = ""; self.deadlock_message = ""
    ...
    def dessiner(self):
        """
        Gère le dessin de l'écran en appelant la méthode d'affichage appropriée
        de l'interface graphique en fonction de l'état actuel du jeu.
        """
        if self.etat_jeu == C.ETAT_MENU_PRINCIPAL:
    ...
    def enregistrer_score(self):
        """
        Enregistre le score du joueur (mouvements et temps) pour le niveau actuel
        s'il est meilleur que le précédent score ou s'il n'y en a pas.
        Met également à jour la liste des niveaux complétés.
        """
    ...
    def passer_au_niveau_suivant_ou_menu(self):
        """
        Après une victoire, ou la fin d'un test de niveau, gère la transition.
        Retourne à l'éditeur si en mode test, sinon à l'écran de sélection de niveau.
        Réinitialise les états pertinents pour la transition.
        """
        if self.mode_test_editeur:
            self.etat_jeu = C.ETAT_EDITEUR
            self.niveau_actuel = None
    ...
    def lancer_resolution_auto(self, type_solveur="bfs"):
        """
        Prépare et lance la visualisation de la résolution du niveau actuel
        en utilisant le type de solveur spécifié (BFS ou Backtracking).
        """
        ...
        if type_solveur == "bfs":
            solv = solveurs.SolveurBFS(etat_init_pour_solveur)
            solution_algo, iterations_algo = solv.resoudre(self.niveau_pour_visualisation)
        elif type_solveur == "backtracking":
            solv = solveurs.SolveurRetourArriere(etat_init_pour_solveur)
            solution_algo, iterations_algo = solv.resoudre(self.niveau_pour_visualisation, C.PROFONDEUR_MAX_BACKTRACKING)
        else:
        ...
        if self.solveur_solution_pas:
            self.niveau_pour_visualisation.appliquer_etat_solveur(etat_init_pour_solveur)

    def avancer_pas_solveur(self):
        """
        Avance d'un pas dans la solution trouvée par le solveur lors de la visualisation.
        Applique le mouvement correspondant au niveau de visualisation.
        """
    ...
    def terminer_visualisation_solveur_rapidement(self):
        """
        Applique tous les pas restants de la solution du solveur d'un coup
        pour atteindre rapidement l'état final de la solution visualisée.
        """
